...
BOOT.OPT <- list(Seed=666,
                 Number.Cluster=12, ## higher if run on a cluster
...
##
GAMLSS.CTRL <- gamlss.control(n.cyc=400) ## long tail for some iterations
...
if( 0 ) {
    ...
    FITTING.SET <- c( paste0(DATATAG, "-", Indexflag, "Transformed") )
